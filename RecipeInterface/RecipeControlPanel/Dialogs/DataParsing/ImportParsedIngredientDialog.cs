...
internal class ImportParsedIngredientDialog : IDialog
{
    private readonly ParsedIngredientImporter ingredientPredictionsImporter;
    ...
    internal ImportParsedIngredientDialog(RecipeContext db, IDialog nextDialog)
    {
        this.nextDialog = nextDialog;
        ingredientPredictionsImporter = new(db, NullLogger.Instance);

        ingredientPredictionsImporter.OnStatusChanged += OnStatusChanged;
        ingredientPredictionsImporter.OnStreamProgressChanged += OnStreamProgressChanged;
        ingredientPredictionsImporter.OnSqlRowsCopied += OnSqlRowsCopied;
    }
    ...
    public async Task<IDialog?> Execute(CancellationToken token = default)
    {
        ...
        await ingredientPredictionsImporter.Import(token);
